...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: `resource created: ${resourceId}` + ` ${resourceName}` + `, available: ${resourceAvailable}` + `, price: ${resourcePrice} € / ${resourcePriceUnit}`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `resource creation failed: ${resourceName} (duplicate)`,
        entityType: "resource",
        entityId: null,
      });
      ...
      return res.status(409).json({
        ...
        details: "A resource with the same name already exists. Please choose another name.",
      });
    }
